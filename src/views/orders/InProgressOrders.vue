        ...
        <div class="flex items-center">
          ...
          <div>
            <!-- <div class="action-icon bg-secondary">
              <q-icon name="done" size="xs"></q-icon>
              <q-tooltip>{{ $t($L.ACTIONS.DONE) }}</q-tooltip>
            </div>
            <div class="action-icon bg-red q-ml-sm">
              <q-icon name="cancel" size="xs" @click="onOpenCancelModal(props.row.id)"></q-icon>
              <q-tooltip>{{ $t($L.ACTIONS.CANCEL) }}</q-tooltip>
            </div> -->
          ...
          <div class="col-6">
            <span class="account-info">
              <q-icon name="mail_outline"
                ><q-tooltip>{{ $t($L.MODEL.ORDER.ACCOUNT_USERNAME) }}</q-tooltip></q-icon
              >
          ...
          <div class="col-6">
            <span class="account-info">
              <q-icon name="lock_outline">
                <q-tooltip>{{ $t($L.MODEL.ORDER.ACCOUNT_PASSWORD) }}</q-tooltip>
              </q-icon>
          ...
          <div class="col-6" v-if="props.row.accountName">
            <span class="account-info">
              <q-icon name="person_outline"
                ><q-tooltip>{{ $t($L.MODEL.ORDER.ACCOUNT_NAME) }}</q-tooltip></q-icon
              >
...
export default defineComponent({
  ...
  setup() {
    ...
    function onOpenDoneModal(id: string, cd: string) {
      isOpenUpload.value = !isOpenCancel.value;
      orderCode.value = cd;
      orderId.value = id;
    }
