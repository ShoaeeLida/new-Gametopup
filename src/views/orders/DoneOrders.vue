  ...
  >
    <template v-slot:item="props">
      ...
      >
        ...
        <button
          type="button"
          class="mt-5 ml-0 inline-flex items-center justify-center rounded-md border border-gray-300 bg-white p-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 sm:mt-0 sm:ml-2"
          @click="
            copyOrder(
              props.row.code,
              props.row.productTitle,
  ...
  setup() {
    ...
    async function takeOrder(id: string) {
      $q.dialog({
        ...
        await fillDataTable(orderService.currentDoneRequestProp);
      });
    }
