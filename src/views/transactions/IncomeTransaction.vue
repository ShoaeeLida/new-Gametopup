...
export default defineComponent({
  ...
  setup() {
    ...
    async function fillDataTable(
      props: RequestProp = transactionService.currentIncomeRequestProp
    ) {
      props.pagination.sortBy = "createDateTime";
      props.pagination.descending = true;
      var response = await transactionService.listIncomeAsync(props);
      quasarTable.value.setData(
        response.data,
        response.total,
        transactionService.currentIncomeRequestProp.pagination
      );
    }
    ...
    onMounted(async () => {
      transactionService.currentIncomeRequestProp.setToFirstPage();
      // eslint-disable-next-line @typescript-eslint/no-floating-promises
      syncDataTable();
    });
    onBeforeUnmount(() => {
