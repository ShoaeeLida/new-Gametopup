<template>
  ...
  <q-page-sticky
    ...
    style="z-index: 11"
  >
    <q-fab v-model="fab2" color="dark" icon="add" @click="openCreate"> </q-fab>
  </q-page-sticky>
  <app-search-modal
...
export default defineComponent({
  ...
  setup() {
    ...
    async function fillDataTable(
      props: RequestProp = transactionService.currentWithdrawRequestProp
    ) {
      props.pagination.sortBy = "createDateTime";
      props.pagination.descending = true;
      var response = await transactionService.listWithdrawAsync(props);
      quasarTable.value.setData(
        response.data,
        response.total,
        transactionService.currentWithdrawRequestProp.pagination
      );
    }
    ...
    onMounted(async () => {
      transactionService.currentWithdrawRequestProp.setToFirstPage();
      // eslint-disable-next-line @typescript-eslint/no-floating-promises
      syncDataTable();
    });
